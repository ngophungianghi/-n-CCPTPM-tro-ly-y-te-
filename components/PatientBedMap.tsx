...
import { subscribeToBeds } from '../services/bedService';
...
export const PatientBedMap: React.FC = () => {
  ...
  useEffect(() => {
    const unsub = subscribeToBeds(setBeds);
    setLoading(false);
    return () => unsub();
  }, []);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {beds.sort((a, b) => a.bedNumber.localeCompare(b.bedNumber)).map(bed => (
          ...
          >
            ...
            {/* Secure Tooltip */}
            {bed.status === 'occupied' && (
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-40 bg-slate-900 text-white p-2 rounded-xl shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-10 text-center">
                <p className="text-[10px] font-bold">Giường đang được sử dụng</p>
              </div>
            )}
          </div>
        ))}
